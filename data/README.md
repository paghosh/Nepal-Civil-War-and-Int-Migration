# Data

This repository does not include raw microdata files due to data-use restrictions.

Raw data are stored locally and synced via Dropbox.

Link to the Dropbox folder for data is:

/Users/rameshdulal/Library/CloudStorage/Dropbox/Nepal Civil Conflict/Results

Please change the path for Data and the Results in the code using Dropbox path.

The dataset "1_conflict_present_absentee_data.dta" has the combined data with casualty from INSEC and Nepal Living Force Survey 2018 (NLFS) 

